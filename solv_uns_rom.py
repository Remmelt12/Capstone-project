...
def solv_prim_uns_galerkin(ubar, V, gen, mu, y0, ode, mass, velo, dvelo, t):
    ...
    def mass_new(y):
        return mass(y, None, msh, phys, disc, 'lmult_transpose', invV)
    def velo_new(y):
        return velo(ubar+np.dot(V, y), None, msh, phys, disc, 'lmult', invV)
    def dvelo_new(y):
        return dvelo(y, None, msh, phys, disc, 'lmult', V)
    ...
    for i, ti in enumerate(t[:-1]):
        ...
        dt = t[i+1] - ti
        y[..., i+1], Ky = ode(mass_new, velo_new, dvelo_new, y[..., i], None, ti, dt, gen, nlsolv)
...
def solv_prim_uns_LSPG(ubar, V, gen, mu, y0, ode, mass, velo, dvelo, t):
    ...
    for i, ti in enumerate(t[:-1]):
        ...
        Qm = np.dot(V, yq[..., i]) # solution at previous timestep

        # Loop over stages 
        for j in range(ode.nstage):
            def nlfunc(yk):
                KQ = np.dot(V, yk[..., j]) # the stage currently being computed
                KQm = np.dot(V, yk[..., :j]) # all previous stages
                return ode.res(mass, velo, KQ, Qm, KQm, None, dt, msh, phys, disc), \
                        ode.jac(mass, dvelo, KQ, Qm, KQm, None, dt, msh, phys, disc, 'rmult', V)

            yk[..., j] = nlsolv(nlfunc, yk)

        # Update solution at current timestep
        yq[..., i+1] = yq[..., i]
        for j in range(ode.nstage):
            yq[..., i+1] += ode.b[j] * yk[..., j]
...
if __name__ == '__main__':
    ...
    from pymortestbed_lite.examples.burg_fv_1d.solv_uns import solv_prim_uns
